...
<body>
	<div id="wrap">
		<div class="container">
			...
			<div id="sub-menu">
				<a href="/ye/admin/adminInfo">전체회원</a> 
			</div>
...
$(document).ready(function() {
	...
	for(var i=0; i<aList.length; i++) {
		...
		str+="<td><button type='button' class='permuteBtn' onclick='memDelete("+aList[i].RN+")'>최종탈퇴</button>";
		str+= "<button type='button' class='secessionBtn' onclick='restore("+aList[i].RN+")'>복구</button></td>";
		str+="</tr>";
	}
	$(".tbody").append(str);
})
...
function restore(index) {
	...
	$.ajax({
		url: '/ye/admin/restoreMem',
		type: 'post',
		data: {id:id},
		success: function(data){
			console.log("성공");
			alert(id+"님의 계정을 복구하였습니다.");
			location.reload();
		},
		error: function(err) {
			console.log("실패");
...
function memDelete(index) {
	var id = $("#memId_"+index).val();
	console.log(id);
	
	var confirmflag = confirm("계정을 삭제하시겠습니까? \n동의하시면 계정은 복구하실 수 없습니다.");
	
	if(confirmflag) {
		$.ajax({
			url: '/ye/admin/deleteMember',
			type: 'post',
			data: {id:id},
			success: function(data){
				console.log("성공");
