...
public class Paging {
	...
	public String makeHtmlPaging() {
		...
		return makeHtmlPaging(currentGroup, totalPage, boardName);
	}

	private String makeHtmlPaging(int currentGroup, int totalPage, String boardName) {
		...
		if (start != 1) {
			sb.append("<a class='prev' href='"+boardName+"?pageNum=" + (start -1) + "'>");
			sb.append("<");
			sb.append("</a>");
		}
